const startBoard = (game, options = { playAgainst: 'human', aiColor: 'black', aiLevel: 'dumb' }) => {
    ...
    const aiPlayer = options.playAgainst === 'ai' ? ai(options.aiColor, options.aiLevel) : null;
    ...
    const checkWarningSign = document.getElementById('checkWarning');
    ...
    game.on('pieceMove', move => {
        const pieceElement = document.getElementById(move.piece.name);
        let from = document.getElementById(move.from);
        const to = document.getElementById(move.piece.position);

        if (move.to !== move.piece.position) {
            const moveToSquare = document.getElementById(move.to);
            if (moveToSquare && pieceElement && moveToSquare.contains(pieceElement)) {
                from = moveToSquare;
            }
        }

        to.append(pieceElement);
        clearSquares();
...
const startNewGame = () => {
    ...
    game.startNewGame(pieces, 'white');
    ...
    const aiLevel = document.querySelector('input[name="ai_level"]:checked')?.value || 'dumb';
...
const showAiLevelSelect = () => document.querySelector('.select-ai-level-container').classList.add('show');
const hideAiLevelSelect = () => document.querySelector('.select-ai-level-container').classList.remove('show');